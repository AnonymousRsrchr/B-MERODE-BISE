...
public class OrderPlacedState extends OrderState {
  ...
  public OrderPlacedState() {
    super("placed", StateType.ONGOING);
  }
  ...
  //--- Modifying Events ---
  @Override
  public void handle_EVrequestShipment(Order object, Context ctx) throws FailedEventHandlingException {
    ...
    OrderShipmentRequestedState newState = new OrderShipmentRequestedState();
    ...
    manufacturer_Manufacturer_Order.getCurrentState().handle_EVrequestShipment(manufacturer_Manufacturer_Order, ctx);
    //Customer
    Customer customer_Customer_Order = (Customer)StubHelper.findBusinessObject(ctx, object.getCustomerId_Customer_Order());
    customer_Customer_Order.getCurrentState().handle_EVrequestShipment(customer_Customer_Order, ctx);
  }

  //--- Ending Events ---
  @Override
  public void handle_EVrejectOrder(Order object, Context ctx) throws FailedEventHandlingException {
    ...
    //Manufacturer
    Manufacturer manufacturer_Manufacturer_Order = (Manufacturer)StubHelper.findBusinessObject(ctx, object.getManufacturerId_Manufacturer_Order());
    manufacturer_Manufacturer_Order.getCurrentState().handle_EVrejectOrder(manufacturer_Manufacturer_Order, ctx);
    //Customer
    Customer customer_Customer_Order = (Customer)StubHelper.findBusinessObject(ctx, object.getCustomerId_Customer_Order());
    customer_Customer_Order.getCurrentState().handle_EVrejectOrder(customer_Customer_Order, ctx);
  }
}
