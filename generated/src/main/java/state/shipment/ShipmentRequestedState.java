...
public class ShipmentRequestedState extends ShipmentState {
  //========== Constructor ===========
  public ShipmentRequestedState() {
    super("requested", StateType.ONGOING);
  }
  ...
  //--- Modifying Events ---
  @Override
  public void handle_EVacceptShipment(Shipment object, Context ctx) throws FailedEventHandlingException {
    ...
    ShipmentWaitingForPickupState newState = new ShipmentWaitingForPickupState();
    ...
    order_Order_Shipment.getCurrentState().handle_EVacceptShipment(order_Order_Shipment, ctx);
    //Carrier
    Carrier carrier_Carrier_Shipment = (Carrier)StubHelper.findBusinessObject(ctx, object.getCarrierId_Carrier_Shipment());
    carrier_Carrier_Shipment.getCurrentState().handle_EVacceptShipment(carrier_Carrier_Shipment, ctx);
  }

  //--- Ending Events ---
  @Override
  public void handle_EVrejectShipment(Shipment object, Context ctx) throws FailedEventHandlingException {
    ...
    //Changes the state of the current Shipment (object)
    ShipmentRejectedState newState = new ShipmentRejectedState();
    object.setCurrentState(newState);
    StubHelper.save(ctx, object);
    //Trigger Event Handling for Each Master
    //Order
    Order order_Order_Shipment = (Order)StubHelper.findBusinessObject(ctx, object.getOrderId_Order_Shipment());
    order_Order_Shipment.getCurrentState().handle_EVrejectShipment(order_Order_Shipment, ctx);
    //Carrier
    Carrier carrier_Carrier_Shipment = (Carrier)StubHelper.findBusinessObject(ctx, object.getCarrierId_Carrier_Shipment());
    carrier_Carrier_Shipment.getCurrentState().handle_EVrejectShipment(carrier_Carrier_Shipment, ctx);
  }
}
